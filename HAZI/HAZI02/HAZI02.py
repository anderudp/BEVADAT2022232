...
def decode_Y(input_arr) -> np.array:
    return np.argmax(input_arr, axis=1)
...
# Ki: 'szilva'
# eval_classification()
def eval_classification(labels: list, probabilities: np.array):
...
# Ki: [-1,2,-1,4,-1,6]
# replace_odd_numbers()
def replace_odd_numbers(input_arr: np.array) -> np.array:
    input_arr[input_arr % 2 != 0] = -1
    return input_arr
...
# Ki: [-1, 1, 1, -1]
# replace_by_value()
def replace_by_value(input_arr: np.array, value) -> np.array:
    input_arr[input_arr < value] = -1
    input_arr[input_arr >= value] = 1
    return input_arr
...
def array_multi(input_arr: np.array):
    return np.prod(input_arr)
...
# Ki: [2, 12]
# array_multi_2d()
def array_multi_2d(input_arr: np.array):
    return np.prod(input_arr, axis=1)
...
# add_border()
def add_border(input_arr: np.array):
    return np.pad(input_arr, pad_width=1, mode="constant", constant_values=0)
